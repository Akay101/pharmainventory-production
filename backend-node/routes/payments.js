...
const PLAN_CONFIG = {
  BASIC: {
    amount: parseInt(process.env.PLAN_BASIC_PRICE) || 20,
    duration_days: 365,
  },
  ADVANCED: {
    amount: parseInt(process.env.PLAN_ADV_PRICE) || 40,
    duration_days: 365,
  },
  AGENTIC: {
    amount: parseInt(process.env.PLAN_AGENTIC_PRICE) || 80,
    duration_days: 365,
  },
};
...
router.post("/create-order", auth, async (req, res, next) => {
  try {
    ...
    const safeFrontendUrl = process.env.FRONTEND_URL
      ? process.env.FRONTEND_URL.replace(/\/$/, "")
      : "";
    const safeBackendUrl = process.env.BACKEND_URL
...
router.post("/create-promo", async (req, res, next) => {
  try {
    ...
    if (!["BASIC", "ADVANCED", "AGENTIC"].includes(plan)) {
      return res
        .status(400)
        .json({ detail: "Invalid plan. Choose BASIC, ADVANCED, or AGENTIC." });
    }
...
router.post("/apply-code", auth, async (req, res, next) => {
  try {
    ...
    const pharmacy = await db
      ...
      .findOne({ id: req.user.pharmacy_id });
    const pharmacyName = pharmacy
      ? pharmacy.name || pharmacy.pharmacy_name || "Unknown"
      : "Unknown";
...
router.post("/verify", auth, async (req, res, next) => {
  try {
    ...
    const verifyPayment = await cashfree
      .PGOrderFetchPayments("2023-08-01", order_id)
      .catch(async () => {
        return await cashfree.PGOrderFetchPayments(order_id);
      });

    const paid = verifyPayment.data?.some(
      (p) => p.payment_status === "SUCCESS"
    );
    if (!paid) {
      return res
        .status(400)
        .json({ detail: "Payment not successful or pending" });
    }

    // 3. Fetch order details to get plan and amount
    const orderDetails = await cashfree
      .PGFetchOrder("2023-08-01", order_id)
      .catch(async () => {
        return await cashfree.PGFetchOrder(order_id);
      });

    const data = orderDetails.data;
    const payment = verifyPayment.data.find(
      (p) => p.payment_status === "SUCCESS"
    );
    ...
    if (!plan) {
      return res
        .status(400)
        .json({ detail: "Could not resolve plan from amount or metadata" });
    }
