...
router.get("/", auth, async (req, res, next) => {
  try {
    ...
    if (low_stock === "true") {
      ...
      allInventory = allInventory.filter((i) =>
        lowStockProducts.includes(i.product_id)
      );
    }
    ...
    if (expiring_soon === "true") {
      ...
      allInventory = allInventory.filter(
        (i) => (i.expiry_date || "9999-12-31") <= cutoff
      );
    }
  ...
  try {
    ...
    let inventory = await db
      ...
      .aggregate([
        ...
        {
          $lookup: {
            from: "suppliers", // make sure this matches your collection name
            localField: "supplier_id", // field in inventory
            foreignField: "id", // ⚠ IMPORTANT: use "id" (not _id) since your IDs are UUID strings
            as: "supplier",
          },
        },
        ...
        {
          $addFields: {
            supplier_name: "$supplier.name", // change if your supplier field is different
          },
        },

        {
          $project: {
            supplier: 0, // remove temporary joined object
            _id: 0,
          },
        },
    ...
    res.json({
      inventory: inventory.slice(0, parsedLimit),
      count: Math.min(inventory.length, parsedLimit),
    });
  } catch (error) {
    next(error);
  }
