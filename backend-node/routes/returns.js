...
// GET /api/returns - List all return bundles
router.get("/", auth, requireSubscription(), async (req, res, next) => {
  try {
    ...
    const total = await db.collection("return_bundles").countDocuments(query);
    const bundles = await db
      .collection("return_bundles")
    ...
    // Fetch unbundled batches marked as Moved to Returns
      ...
      .aggregate([
        {
          $match: {
            ...
            return_status: "Moved to Returns",
            $or: [
              { return_bundle_id: null },
              { return_bundle_id: "" },
              { return_bundle_id: { $exists: false } },
            ],
          },
        },
        {
...
// GET /api/returns/batches - Browse NON-EXPIRED batches from inventory
router.get("/batches", auth, requireSubscription(), async (req, res, next) => {
  try {
    ...
    // STRICT: Only non-expired items or moved to returns items
    const matchQuery = {
      pharmacy_id: req.user.pharmacy_id,
      $or: [
        ...
        { expiry_date: "" },
        { expiry_date: "-" },
        { return_status: { $in: ["Moved to Returns", "Picked", "Returned"] } },
      ],
    };
    ...
    if (status_filter === "unmoved") {
      matchQuery.return_status = { $in: [null, "", "Not Moved"] };
      matchQuery.available_quantity = { $gt: 0 };
    } else if (status_filter === "moved") {
      matchQuery.return_status = { $in: ["Moved to Returns", "Picked"] };
    } else if (status_filter === "returned") {
      matchQuery.return_status = "Returned";
    }
    ...
    const batches = await db
      ...
      .aggregate([
        ...
        { $sort: { product_name: 1, created_at: -1 } },
      ])
      .toArray();
...
// GET /api/returns/:id - Get single return bundle
router.get("/:id", auth, requireSubscription(), async (req, res, next) => {
  try {
    const db = mongoose.connection.db;
    const bundle = await db.collection("return_bundles").findOne(
      { id: req.params.id, pharmacy_id: req.user.pharmacy_id },
      { projection: { _id: 0 } }
    );

    if (!bundle) {
      return res.status(404).json({ detail: "Return bundle not found" });
    }
...
// POST /api/returns - Create new return bundle
router.post("/", auth, requireSubscription(), async (req, res, next) => {
  ...
  try {
    ...
    // Generate bundle number RET-YYYYMM-XXXX
    const datePrefix = new Date().toISOString().slice(0, 7).replace("-", "");
    const count = await db.collection("return_bundles").countDocuments({
      pharmacy_id: req.user.pharmacy_id,
    });
    const bundleNo = `RET-${datePrefix}-${String(count + 1).padStart(4, "0")}`;
    ...
    const processedItems = items.map((item) => {
      ...
      return {
        ...
        reason: item.reason || reason || "General Return",
        notes: item.notes || "",
      };
    });
    ...
    const bundleData = {
      ...
      status: status || "Draft",
      return_type: "General Return",
      reason: reason || "General Return",
      return_date,
    ...
    await db.collection("return_bundles").insertOne(bundleData, { session });

    // Mark matching inventory batches as "Moved to Returns"
      ...
      if (item.inventory_id) {
        await db.collection("inventory").updateOne(
          { id: item.inventory_id, pharmacy_id: req.user.pharmacy_id },
          {
            $set: {
              return_status: "Moved to Returns",
              return_bundle_id: bundleId,
              return_bundle_no: bundleNo,
              updated_at: new Date().toISOString(),
            },
          },
          { session }
    ...
    await logActivity({
      ...
      action: "CREATE_RETURN_BUNDLE",
      details: `Created Purchase Return Note ${bundleNo} for supplier ${supplier_name} with ${processedItems.length} items (₹${bundleData.total_amount})`,
    });
...
// POST /api/returns/move-batch - Quick action to mark a batch as moved to returns
router.post("/move-batch", auth, requireSubscription(), async (req, res, next) => {
  try {
    ...
    res.json({ message: "Batch marked as Moved to Returns", batch_id: batch.id });
  } catch (error) {
    next(error);
  }
});

// PATCH /api/returns/:id/status - Advance or update status
router.patch("/:id/status", auth, requireSubscription(), async (req, res, next) => {
  ...
  try {
    ...
    if (status === "Payment Done (Returned)") {
      ...
      // Deduct/Zero out inventory stock for these batches & mark Returned
    ...
    await db.collection("return_bundles").updateOne(
      { id: bundle.id },
      { $set: updateFields },
      { session }
    );
    ...
    await logActivity({
      ...
      details: `Updated Return Bundle ${bundle.bundle_no} status to "${status}"`,
    });
...
// POST /api/returns/:id/payments - Record partial or full payment
router.post("/:id/payments", auth, requireSubscription(), async (req, res, next) => {
  ...
  try {
    ...
    await logActivity({
      ...
      user_id: req.user.id,
      user_name: req.user.name,
      action: "RECORD_RETURN_PAYMENT",
      details: `Recorded payment of ₹${payAmount} via ${payment_mode} for Return Bundle ${bundle.bundle_no}`,
    });
...
// DELETE /api/returns/:id - Cancel/Delete draft return bundle
router.delete("/:id", auth, requireSubscription(), async (req, res, next) => {
  ...
  try {
    ...
    await db.collection("return_bundles").deleteOne({ id: bundle.id }, { session });

    await session.commitTransaction();
    session.endSession();

    res.json({ message: "Return bundle deleted and inventory batches restored" });
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    next(error);
