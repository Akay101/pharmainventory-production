...
export default function ReturnsPage() {
  ...
  // Non-expired Inventory Batches tab state
  ...
  const [bundleReason, setBundleReason] = useState("Excess Stock");
  ...
  // When supplier changes in create modal, load their available non-expired batches
  useEffect(() => {
  ...
  const fetchBundles = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch return bundles:", e);
      toast.error("Failed to load purchase return notes");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchAvailableBatches = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/returns/batches?${params.toString()}`);
  ...
  // Fetch all non-expired/moved batches for a specific supplier
  const fetchBatchesForSelectedSupplier = async (supId) => {
    setLoadingSupplierBatches(true);
    try {
      const res = await axios.get(`${API}/returns/batches?supplier_id=${supId}`);
      const unbundledForSup = (res.data.batches || []).filter(
        (b) => b.return_status !== "Returned"
      );
      ...
      if (bundleItemsDraft.length === 0 && unbundledForSup.length > 0) {
        setBundleItemsDraft(
          unbundledForSup.map((b) => ({
            ...
            reason: bundleReason || "Excess Stock",
            notes: "",
          }))
        );
      }
  ...
  const handleSelectAllBatches = () => {
    if (selectedBatchIds.size === availableBatches.length) {
      setSelectedBatchIds(new Set());
    } else {
      setSelectedBatchIds(new Set(availableBatches.map((b) => b.id)));
    }
  };
  ...
  const handleStartCreateBundle = (preselectedSupplierId = null, preselectedItems = null) => {
    if (preselectedSupplierId) {
      ...
      const selectedList = availableBatches.filter((b) => selectedBatchIds.has(b.id));
  ...
  const handleToggleBatchInDraft = (batch) => {
    setBundleItemsDraft((prev) => {
      const exists = prev.some(
        (it) => it.inventory_id === batch.id || (it.batch_no === batch.batch_no && it.product_name === batch.product_name)
      );
      if (exists) {
        return prev.filter(
          (it) => !(it.inventory_id === batch.id || (it.batch_no === batch.batch_no && it.product_name === batch.product_name))
  ...
  const handleQuickMoveBatch = async (batch) => {
    try {
      ...
      toast.success(`Batch ${batch.batch_no} marked as Moved to Returns`, {
        action: {
  ...
  const handleCreateBundleSubmit = async () => {
    ...
    if (bundleItemsDraft.length === 0) {
      toast.error("Please add at least one batch to the return bundle");
      return;
    }
    ...
    try {
      ...
      toast.success(`Purchase Return Note ${res.data.bundle_no} created successfully!`);
  ...
  const handleUpdateStatus = async (bundleId, newStatus) => {
    try {
      await axios.patch(`${API}/returns/${bundleId}/status`, {
        status: newStatus,
      });
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      const res = await axios.post(
        `${API}/returns/${targetBundleForPayment.id}/payments`,
        {
          amount: amt,
  ...
  const handleDeleteBundle = async () => {
    ...
    try {
      await axios.delete(`${API}/returns/${bundleToDelete.id}`);
      toast.success("Purchase Return Note deleted and batches restored");
  ...
  // Group unbundled batches by supplier for the top quick-bundle card
  ...
  return (
    <div className="space-y-4 pb-12 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-border/60">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-muted-foreground">
                Process non-expired inventory returns, damaged stock, and supplier credit settlements
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => (activeTab === "bundles" ? fetchBundles() : fetchAvailableBatches())}
          ...
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] font-semibold text-muted-foreground mt-0.5">
                {stats.total_items} items in return queue
              </p>
            </div>
      ...
      {/* PENDING UNBUNDLED BATCHES CALLOUT */}
      {unbundledSupplierGroups.length > 0 && (
        <div className="p-4 rounded-2xl bg-gradient-to-r from-blue-500/15 via-orange-500/10 to-transparent border border-blue-500/30 space-y-3">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <Clock className="w-5 h-5 text-blue-500" />
              <div>
                <h3 className="text-xs sm:text-sm font-black text-foreground">
                  Unbundled Non-Expired Return Batches ({unbundledBatches.length} items ready for return notes)
                </h3>
                <p className="text-[11px] text-muted-foreground">
                  Batches marked for return. Click below to generate return notes for each supplier.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex items-center gap-2 border-b border-border/60">
        ...
        >
          ...
          <span>Purchase Return Notes ({totalBundles})</span>
        </button>
        ...
        >
          ...
          <span>Browse Non-Expired Inventory Batches</span>
          {availableBatches.length > 0 && (
            <Badge className="px-1.5 py-0 text-[10px] font-extrabold bg-blue-500/20 text-blue-600 dark:text-blue-400 border-blue-500/30">
            ...
            <div className="flex items-center gap-2 flex-1 min-w-[240px]">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder="Search by Return Note #, Supplier, Product, Batch..."
                ...
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={9} className="py-12 text-center">
                      <Loader size="md" text="Loading Purchase Return Notes..." />
                    </TableCell>
                  </TableRow>
                ) : bundles.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={9} className="py-12 text-center text-muted-foreground">
                      <Undo2 className="w-10 h-10 mx-auto mb-2 opacity-40 text-muted-foreground" />
                      <p className="text-xs font-bold uppercase tracking-widest">No Purchase Return Notes found</p>
                      <p className="text-[11px] text-muted-foreground mt-1">
                        Select non-expired inventory batches to start a return or credit request.
                      </p>
                    </TableCell>
                  </TableRow>
                ) : (
                      ...
                      >
                        ...
                        <TableCell>
                          <div className="text-xs">
                            ...
                            <Badge variant="outline" className="text-[9px] px-1.5 py-0 mt-0.5 font-bold">
                              {bundle.reason || "General Return"}
                            </Badge>
                          </div>
                        </TableCell>
      ...
      {/* TAB 2: BROWSE NON-EXPIRED INVENTORY BATCHES */}
      {activeTab === "inventory" && (
        <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 flex-1 min-w-[240px]">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder="Search non-expired batches by Product, Batch #, Salt, MFG..."
                  className="pl-9 h-9 text-xs rounded-xl bg-background border-border/70 font-semibold"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <Select value={batchStatusFilter} onValueChange={setBatchStatusFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="moved">Moved to Returns</SelectItem>
              ...
              {selectedBatchIds.size > 0 && (
                ...
                >
                  ...
                  Return {selectedBatchIds.size} Selected
                </Button>
              )}
            </div>
          </div>
                ...
                <TableRow className="bg-muted/40 hover:bg-muted/40">
                  <TableHead className="w-10 text-center">
                    <input
                      ...
                      checked={
                        availableBatches.length > 0 &&
                        selectedBatchIds.size === availableBatches.length
                      }
                ...
                {loadingBatches ? (
                  <TableRow>
                    <TableCell colSpan={10} className="py-12 text-center">
                      <Loader size="md" text="Loading non-expired batches..." />
                    </TableCell>
                  </TableRow>
                ) : availableBatches.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={10} className="py-12 text-center text-muted-foreground">
                      <Package className="w-10 h-10 mx-auto mb-2 opacity-40 text-muted-foreground" />
                      <p className="text-xs font-bold uppercase tracking-widest text-foreground">
                        No Non-Expired Batches Found
                      </p>
                    </TableCell>
                  </TableRow>
                ) : (
                  availableBatches.map((b) => {
                    ...
                    const isMoved = b.return_status === "Moved to Returns" || b.return_status === "Picked";
                    const isReturned = b.return_status === "Returned";
                        ...
                        <TableCell className="text-center">
                          {isReturned ? (
                            ...
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-black bg-amber-500/15 text-amber-500 border border-amber-500/30">
                              ...
                              Moved to Returns {b.return_bundle_no ? `(#${b.return_bundle_no})` : ""}
                            </span>
                          ) : (
      ...
      {/* CREATE RETURN BUNDLE MODAL WITH INLINE BATCH PICKER */}
        ...
        <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto rounded-3xl p-6 bg-background/95 backdrop-blur-xl border-border/80">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-muted-foreground">
              Bundle non-expired inventory batches for supplier credit, recall, or return.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                <Label className="text-xs font-bold">Return Reason</Label>
                <Select value={bundleReason} onValueChange={setBundleReason}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Near Expiry Recall">Near Expiry Recall</SelectItem>
                    <SelectItem value="Slow Moving">Slow Moving</SelectItem>
                    <SelectItem value="Supplier Callback">Supplier Callback</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-card/60 border border-border/70 space-y-2.5">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-black text-foreground flex items-center gap-1.5">
                    ...
                    Available Batches for {suppliers.find((s) => s.id === selectedSupplierForBundle)?.name || "Supplier"} ({supplierAvailableBatches.length})
                  </p>
                ...
                {loadingSupplierBatches ? (
                  ...
                  <div className="p-4 text-center rounded-xl bg-muted/20 border border-border/40 text-xs text-muted-foreground">
                    No unbundled non-expired batches currently found for this supplier.
                  </div>
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div className="truncate">
                            ...
                            <p className="text-[10px] font-mono text-muted-foreground mt-0.5">
                              Batch: {b.batch_no} • Exp: {b.expiry_date || "-"} • {b.available_quantity || 0}u
                            </p>
                          </div>
            ...
            <div className="space-y-2">
              <Label className="text-xs font-bold">
                Bundled Batches ({bundleItemsDraft.length})
              </Label>
              ...
              {bundleItemsDraft.length === 0 ? (
                <div className="p-6 text-center border border-dashed border-border/70 rounded-2xl bg-muted/20 text-xs font-semibold text-muted-foreground">
                  No batches selected. Please select batches from the "Browse Non-Expired Inventory" tab.
                </div>
              ) : (
                <div className="rounded-xl border border-border/70 overflow-hidden max-h-60 overflow-y-auto">
                  <Table>
                    <TableHeader>
              ...
              {bundleItemsDraft.length > 0 && (
                <div className="flex justify-end pt-2">
                  <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl px-4 py-2 text-right">
                    <span className="text-xs font-bold text-muted-foreground mr-2">Total Return Value:</span>
            ...
            <div>
              <Label className="text-xs font-bold">Notes / Reason Details</Label>
              <Textarea
                value={bundleNotes}
                onChange={(e) => setBundleNotes(e.target.value)}
                placeholder="Add supplier credit terms or remarks..."
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            >
              {creatingBundle ? "Creating..." : "Save Return Note"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {detailBundle && (
            <div className="space-y-5">
              <DialogHeader>
                ...
                <DialogDescription className="text-xs text-muted-foreground">
                  Supplier: <strong className="text-foreground">{detailBundle.supplier_name}</strong> | Reason:{" "}
                  <strong>{detailBundle.reason || "General Return"}</strong> | Created:{" "}
                    ...
                    <TableBody>
                      {detailBundle.items?.map((it, idx) => (
                        <TableRow key={idx} className="text-xs">
                          ...
                          <TableCell className="font-mono">{it.expiry_date || "-"}</TableCell>
              ...
              <div className="p-4 rounded-2xl bg-card/80 border border-border/70 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="p-2.5 rounded-xl bg-muted/40">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase">Total Return Value</p>
      ...
      <Dialog open={paymentModalOpen} onOpenChange={setPaymentModalOpen}>
        <DialogContent className="max-w-md rounded-3xl p-6 bg-background/95 backdrop-blur-xl border-border/80">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-muted-foreground">
              Record reimbursement for Return Note {targetBundleForPayment?.bundle_no}
            </DialogDescription>
          </DialogHeader>
