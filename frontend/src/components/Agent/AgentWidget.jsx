...
export default function AgentWidget({ user }) {
  ...
  const handleSwitchConversation = async (convId) => {
    try {
      ...
      const lastDraftMsg = [...res.data.messages]
        .reverse()
        .find((m) => m.draftState);
        ...
        if (data.type === "action" && data.action) {
          ...
          setMessages((prev) => [
            ...prev,
            {
              role: "model",
              text:
                data.content || "I couldn't process that. Please try again.",
              viewType: data.viewType || null,
              purchasesData: data.purchasesData || null,
              draftState: data.draftState || null,
              chips: data.chips || [],
  ...
  const handleExecuteAction = async (directActionPayload = null) => {
    ...
    try {
      ...
      if (
        actionToRun.function_name === "create_purchase" ||
        actionToRun.intent === "create_purchase"
      ) {
        endpoint = `${API}/purchases`;
      } else if (
        actionToRun.function_name === "list_purchases" ||
        actionToRun.intent === "list_purchases"
      ) {
        endpoint = `${API}/purchases`;
        method = "get";
        payload = { params: payload };
      } else if (
        actionToRun.function_name === "delete_purchase" ||
        actionToRun.intent === "delete_purchase"
      ) {
        endpoint = `${API}/purchases/${payload.purchase_id}`;
        method = "delete";
        payload = {};
      } else if (
        actionToRun.function_name === "check_price_history" ||
        actionToRun.intent === "check_price_history"
      ) {
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-background relative">
              ...
              <ChatInputBar
                ...
                onOpenFollowup={() =>
                  setFollowupModal({
                    open: true,
                    question: "Provide product, supplier & quantity:",
                  })
                }
              />
            </div>
